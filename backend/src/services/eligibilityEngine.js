function checkLevelMatch(level, rule) {
...
function checkEligibility(member, family, rule, scheme) {
  const matchedCriteria = [];
  const missingCriteria = [];
  let isEligible = true;

  const age = calculateAge(member.dob);
  const income = family ? family.totalIncome : ((member.monthlyIncome || 0) * 12);
  ...
  const requiredDocList = scheme && scheme.requiredDocuments ? scheme.requiredDocuments : ["aadhaar_card", "income_cert"];
